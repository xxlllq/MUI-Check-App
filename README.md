#  :iphone: MUI-Check-App
使用MUI开发的一款用于检查打分的App，主要功能为数据采集（音频、视频、照片采集），并将数据上传到服务器进行相关分析，Android与iOS真机测试，有一些敏感数据所以对图片进行了模糊。

- **首页**

首页主要用于展示分配给当前用户的检查任务，主数据为任务，点击展开则为详细检查任务子数据。支持下拉刷新，上拉加载数据列表功能。
<div align="center">
<kbd>
  <div style="border: 1px solid gainsboro;border-radius: 5px;" align="center">
    <img width="100%" height="auto" src="https://raw.githubusercontent.com/xxlllq/MUI-Check-App/master/app/Screenshot_20190325-110920.png" alt="First Picture" title="First Picture"/>
  </div>
</kbd>
  <kbd>
  <div style="border: 1px solid gainsboro;border-radius: 5px;" align="center">
    <img width="100%" height="auto" src="https://raw.githubusercontent.com/xxlllq/MUI-Check-App/master/app/Screenshot_20190326-095552.png" alt="Second Picture" title="Second Picture"/>
  </div>
</kbd>
 </div>

- **检查页**

检查页主要用于音频、视频、照片等数据采集，实现的功能有调用摄像头进行拍照与查看拍照后的图片、录像与录像播放、录音以及播放音频。用户可将采集的数据上传到服务器，同时也支持用户从服务器远程播放音频（首次查看从服务器缓存在本地后播放，以后就直接播放本地缓存的音频文件）、视频、照片。
<div align="center">
<kbd>
  <div style="border: 1px solid gainsboro;border-radius: 5px;" align="center">
    <img width="100%" height="auto" src="https://raw.githubusercontent.com/xxlllq/MUI-Check-App/master/app/Screenshot_20190326-095645.png" alt="First Picture" title="First Picture"/>
  </div>
</kbd>
<kbd>
  <div style="border: 1px solid gainsboro;border-radius: 5px;" align="center">
    <img width="100%" height="auto" src="https://raw.githubusercontent.com/xxlllq/MUI-Check-App/master/app/Screenshot_20190326-095708.png" alt="Second Picture" title="Second Picture"/>
  </div>
</kbd>
<kbd>
  <div style="border: 1px solid gainsboro;border-radius: 5px;" align="center">
    <img width="100%" height="auto" src="https://raw.githubusercontent.com/xxlllq/MUI-Check-App/master/app/Screenshot_20190326-095719.png" alt="Third Picture" title="Third Picture"/>
  </div>
</kbd>
 </div>
 
- **其他**

登录、退出登录
<div align="center">
<kbd>
  <div style="border: 1px solid gainsboro;border-radius: 5px;" align="center">
    <img width="100%" height="auto" src="https://raw.githubusercontent.com/xxlllq/MUI-Check-App/master/app/Screenshot_20190326-101649.png" alt="First Picture" title="First Picture"/>
  </div>
</kbd>
<kbd>
  <div style="border: 1px solid gainsboro;border-radius: 5px;" align="center">
    <img width="100%" height="auto" src="https://raw.githubusercontent.com/xxlllq/MUI-Check-App/master/app/Screenshot_20190326-101638.png" alt="Second Picture" title="Second Picture"/>
  </div>
</kbd>
 </div>
